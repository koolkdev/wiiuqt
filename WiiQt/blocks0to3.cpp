...
Blocks0to3::Blocks0to3( const QList<QByteArray> &blocks )
{
    _ok = false;
    if( !blocks.isEmpty() )
        SetBlocks( blocks );
...
bool Blocks0to3::SetBlocks( const QList<QByteArray> &blocks )
{
    _ok = false;
    ...
    for( quint16 i = 0; i < cnt; i++ )
    {
        ...
        {
            qWarning() << "Blocks0to3::SetBlocks -> block" << i << "is" << hex << blocks.at( i ).size() << "bytes";
            return false;
        }
    }
    this->blocks = blocks;
...
bool Blocks0to3::CheckBoot1()
{
    ...
    if ( blocks.at(0) != blocks.at(1) ){
        qWarning() << "Blocks0to3::CheckBoot1 -> Boot0 block 0 and block 1 not identical!";
        return false;
    }
    ...
    if ( boot1Info.boot1Size % 0x1000 || boot1Info.boot1Size > 0xF000 ) {
        qWarning() << "Blocks0to3::CheckBoot1 -> Invalid boot1 size" << boot1Info.boot1Size;
        return false;
    }

    if ( boot1Info.rsaKeyIndex != 2 ) {
        qWarning() << "Blocks0to3::CheckBoot1 -> Invalid RSA key index" << boot1Info.rsaKeyIndex;
        return false;
    }

    if ( boot1Info.zero ) {
        qWarning() << "Blocks0to3::CheckBoot1 -> Should be zero" << boot1Info.zero;
        return false;
    }

    if ( boot1Info.unknownType != 0x21 || boot1Info.unknownType != 0x21 ) {
        qWarning() << "Blocks0to3::CheckBoot1 -> Invalid something type" << boot1Info.unknownType;
        return false;
    }
